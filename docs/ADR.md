...
For the agent harness we will use langgraph, no persistence needed for now. The LLM is Groq via `langchain-groq` (`ChatGroq`), with model name from config (default `llama-3.3-70b-versatile`). The agent has access to a run_python_script tool with ifcopenshell available and other necessary dependencies.
