...
The system takes an IFC file and a number of BCF incidences and feeds them to an AI agent that is capable of understanding what the problems are and interacting with the IFC file to solve them.

For the interaction with the IFC we have chosen ifcopenshell, as it is the most comprehensive and capable.

The application is written in python. And it will work locally. Local access and changes to the IFC file in a folder prefilled manually by the person invoking the agent with the IFC file. By using ifctester to validate the IFC, it will produce BCF files, be able to read them and produce fixes over a new copy of the IFC file, beside a summary of the fixes for human review.
...
An extra feature is the ability to visualize the diff between the original IFC file and the result IFC produced by the AI agent after applying the fixes. We won't tackle this now, but it will be next in our roadmap. For it we will use ifc-lite (https://github.com/louistrue/ifc-lite), a fast open source IFC visualizer.
